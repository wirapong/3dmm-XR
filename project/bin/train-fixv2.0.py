...
LR = 5e-5  # 学习率
CLIP = 5  # 梯度裁剪
TRAIN_DATA_PATH = r'F:\KKU IS REASEARCH\project\data\train\\'  # 训练集目录
TEST_DATA_PATH = r'F:\KKU IS REASEARCH\project\data\test\\'  # 测试集目录
EXTENSION = 'jpg'  # 图片后缀名
LOG_PATH = None  # 日志路径
SAVE_PATH = r'F:\KKU IS REASEARCH\project\model'  # 模型保存路径
MODEL_NAME = 'fit_model'  # 模型名
LAST_EPOCH = -1  # 上一次运行的EPOCH
PER_EPOCH_SAVE = 10  # 每多少EPOCH储存一次
PER_EPOCH_TEST = 2  # 每多少EPOCH测试一次
SEED = 163  # 随机种子
INPUT_SIZE = 120  # 输入图像大小
MAX_INDEX = INPUT_SIZE - 1  # 最大有效索引值

BFM_PATH = r'F:\KKU IS REASEARCH\project\model\BFM.mat'  # BFM模型位置
SHAPE_NUM = 40  # 形状参数数量
EXP_NUM = 10  # 表情参数数量
...
L2DCON_WEIGHTS = torch.as_tensor([1, 1, 1, 1, 1, 1, 1, 2, 1, 1, 2, 1, 1, 4, 1, 2, 4, 2], dtype=torch.float32,
                                 device=DEVICE)
...
class ImageDataset(Dataset):
    def __init__(self, filePath, extension='jpg') -> None:
        super().__init__()
        self.imagePaths = glob.glob(f"{filePath}*.{extension}")
        self.labelPaths = glob.glob(f"{filePath}*.mat")

        # 确保图像和标签数量匹配
        assert len(self.imagePaths) == len(self.labelPaths), \
            f"图像数量 {len(self.imagePaths)} 与标签数量 {len(self.labelPaths)} 不匹配"
    ...
    def __getitem__(self, index):
        ...
        # 裁剪坐标值到有效范围 [0, MAX_INDEX]
        x2d[0] = np.clip(x2d[0], 0, MAX_INDEX)  # x坐标
        x2d[1] = np.clip(x2d[1], 0, MAX_INDEX)  # y坐标
        ...
        try:
            ...
            print(f"处理图像 {self.imagePaths[index]} 时出错:")
            print(f"x2d坐标: {x2d}")
            print(
                f"维度信息: flms形状 {flms.shape}, x2d[1]范围 {x2d[1].min()}-{x2d[1].max()}, x2d[0]范围 {x2d[0].min()}-{x2d[0].max()}")
            raise e
...
def main():
    # 数据集载入
    trainSet = ImageDataset(TRAIN_DATA_PATH, EXTENSION)
    testSet = ImageDataset(TEST_DATA_PATH, EXTENSION)

    print(f'成功载入训练集，路径: {TRAIN_DATA_PATH}，大小为: {len(trainSet)}')
    print(f'成功载入测试集，路径: {TEST_DATA_PATH}，大小为: {len(testSet)}')
    ...
    with SummaryWriter(LOG_PATH, comment=f'-Lr_{LR:e}-Bacth_{BATCH_SIZE}', purge_step=LAST_EPOCH) as writer:
        ...
        for epoch in range(LAST_EPOCH + 1, EPOCH_NUM):
            ...
            for i, data in enumerate(trainLoader, 0):
                ...
                # 确保索引在有效范围内
                ind = torch.clamp(y2d.int(), 0, MAX_INDEX)
                indx = ind[:, :, 0].flatten()
                indy = ind[:, :, 1].flatten()
            ...
            print(
                f'Epoch: {epoch:<6}  用时: {end - start:<.2f}s  Loss: {running_loss:<6e}  Train-R2: {running_r2:.6f}  Test-R2: {testing_r2:.6f}  Lr: {lr:<6e}')
